...
/**
 * Gets a random value from an array of arrays
 * @param {*} array The array of arrays
 * @returns The random value
 */
function GetValue(array) {
	if (array.length > 1) return RandomValue(RandomValue(array));
	else                  return RandomValue(array[0]);
}
...
/**
 * Changes the formatting of values from Discord markdown to HTML
...
function FixFormat(result) {
	result = result.replace(/\n/g, "<br>")                                         // change \n to <br>
			       ...
			       .replace(/\*\*(.*?)\*\*/g, "<b>$1</b>")                         // change **...** to <b>...</b>
			       .replace(/\*(.*?)\*/g, "<i>$1</i>")                             // change *...* to <i>...</i>
			       .replace(/\|\|(.*?)\|\|/g, "<span class=\"spoiler\">$1</span>") // change ||...|| to <span class="spoiler">...</span>
			       .replace(/~~(.*?)~~/g, "<s>$1</s>")                             // change ~~...~~ to <s>...</s>
				   .replace(/\?\./g, "?");                                         // change ?. to ?
...
/**
 * Returns true or false based on a percentage, e.g. chanceTrue(50) has a 50% chance of returning true
 * @param {number} percent The percentage
 * @returns True or false
 */
function chanceTrue(percent) {
